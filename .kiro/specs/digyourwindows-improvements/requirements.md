...
DigYourWindows 是一个 Windows 系统深度诊断工具，目前以 C# WPF 版本为主（仓库内不再包含旧的 CLI 版本）。本需求文档旨在规划项目的改进方向，解决当前存在的架构问题、代码质量问题，并增强功能完整性和用户体验。

## Glossary

- **System**: DigYourWindows 诊断工具（C# WPF 版本）
- **WPF_GUI**: 基于 C# WPF 开发的图形界面版本，提供交互式诊断
- **WMI**: Windows Management Instrumentation，Windows 管理工具接口
- **WMIC**: WMI 命令行工具（已被 Microsoft 标记为弃用；项目不应依赖 WMIC）
...
1. WHEN the System accesses WMI data THEN the System SHALL use native .NET/WMI APIs (e.g., `System.Management`) instead of spawning WMIC processes
2. WHEN the System generates reports THEN the System SHALL separate data collection logic from report generation logic into distinct modules
...
4. WHEN the System encounters WMI access failures THEN the System SHALL provide actionable error messages indicating the specific permission or configuration issue
5. WHERE data models are shared between Core services and UI THEN the System SHALL use a single standardized contract to avoid duplicate model definitions
...
3. WHEN the System processes reliability records THEN the System SHALL use efficient data structures to minimize memory allocation
...
4. WHEN the System generates reports THEN the System SHALL include a summary section highlighting critical issues at the top
...
### Requirement 6: 报告导入/导出与兼容性

**User Story:** 作为用户，我希望能够导出诊断报告并在之后导入查看，以便离线分析、分享与复现问题。

#### Acceptance Criteria

1. WHEN the System exports diagnostic data to JSON THEN the System SHALL follow the standardized `DiagnosticData` schema and include a collection timestamp
2. WHEN the WPF_GUI imports a previously exported JSON report THEN the System SHALL display it without errors and populate all UI sections
3. WHEN the System serializes diagnostic data to JSON THEN the System SHALL ensure deserialization produces equivalent data structures
4. WHEN the JSON schema evolves THEN the System SHALL remain backward compatible for at least one minor version, or provide clear error messages
5. WHEN the System exports HTML reports THEN the System SHALL use consistent template structure and styling across releases
...
5. WHEN a developer sets up the development environment THEN the System SHALL provide setup scripts and dependency documentation for the C#/.NET environment

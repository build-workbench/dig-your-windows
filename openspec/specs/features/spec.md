...
- THEN 返回 `PerformanceAnalysisData` 含 `OverallScore` (0-100)
- AND 返回四个子分：`StabilityScore`, `PerformanceScore`, `MemoryScore`, `DiskScore`（均 0-100）
- AND `OverallScore = round(Stability×0.4 + Performance×0.3 + Memory×0.15 + Disk×0.15)`

### Scenario: 评分边界值
- GIVEN 所有传感器数据处于最差状态
- THEN `OverallScore` = 0，不出现负数
- GIVEN 所有传感器数据处于最佳状态
- THEN `OverallScore` = 100，不超过 100
